...
Write-Log "" 'INFO'
$testBody = '{"status":"successful","name":"test"}'
Write-Log "  Test the listener (after starting):" 'INFO'
Write-Log "    Invoke-RestMethod http://localhost:${Port}/health" 'INFO'
Write-Log "    Invoke-RestMethod -Method POST -Uri http://localhost:${Port}/trigger -ContentType 'application/json' -Body '$testBody'" 'INFO'
Write-Log "" 'INFO'
